<template>
  <v-data-table :headers="headers" item-value="change_quantity_id" :items="transactions" :search="search"
        ...
        <div v-for="(transaction, ix) in transactions" :key="ix">
          <v-row dense>
            <v-col cols="12" md="6" sm="6">
              <v-select variant="solo" item-value="product_id" item-text="product_name" :items="products" 
                label="Product Name" v-model="transaction.product_id" :rules="[rules.required]"
                density="comfortable"></v-select>
            </v-col>
            <v-col cols="12" md="2" sm="2">
              <v-text-field label="Total" variant="solo" v-model="transaction.quantity" :rules="[rules.required]"
                :error-messages="err.retail_price ? err.retail_price[0] : ''" density="comfortable"></v-text-field>
            </v-col>
            <v-col cols="12" md="2" sm="2">
              <v-text-field label="Error" variant="solo" v-model="transaction.error" :rules="[rules.required]"
                :error-messages="err.retail_price ? err.retail_price[0] : ''" density="comfortable"></v-text-field>
            </v-col>
            <v-col cols="12" md="2" sm="2">
              <v-text-field label="Retail Price" variant="solo" density="comfortable" disabled v-model="transaction.fine"></v-text-field>
            </v-col>
          </v-row>
        </div>
