          ...
          <v-sheet>
            <v-row dense>
              <v-col cols="12" md="12" sm="12">
                <v-card title="Scope" class="bg-light-blue-darken-3">
                  <v-card-text>
                    <v-select
...
export default {
  ...
  methods: {
    loadProducts() {
      this.loading = true;
      axios.get("/fetch-reports").then((res) => {
        this.products = res.data;
      });
      axios.get("/fetch-reports-total").then((res) => {
    ...
    filterProducts() {
      ...
      if (this.filter.scope == 0 && this.filter.range == "All") {
        ...
        axios.get("/fetch-reports/" + this.filter.scope).then((res) => {
          this.products = res.data;
        });
        axios.get("/fetch-reports-total/" + this.filter.scope).then((res) => {
          this.totals = res.data;
        });
      } else if (
        this.filter.scope !== 0 &&
        ...
        axios
          .get("/fetch-reports/" + this.filter.scope + "/" + this.filter.to)
          .then((res) => {
            this.products = res.data;
          });
        axios
          .get("/fetch-reports-total/" + this.filter.scope + "/" + this.filter.to)
          .then((res) => {
            this.totals = res.data;
          });
      } else if (
        ...
        axios.get("/fetch-reports-date/" + this.filter.to).then((res) => {
          this.products = res.data;
        });
        axios.get("/fetch-reports-total-date/" + this.filter.to).then((res) => {
          this.totals = res.data;
        });
      }
      else if (
